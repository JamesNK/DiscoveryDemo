...
public sealed class ResolverResult
{
    ...
    [DebuggerStepThrough]
    private ResolverResult(Status status, IReadOnlyList<BalancerAddress>? addresses)
    {
        Status = status;
        Addresses = addresses;
    }
    ...
    public static ResolverResult ForFailure(Status status)
    {
        ...
        return new ResolverResult(status, addresses: null);
    }
    ...
    {
        return new ResolverResult(Status.DefaultSuccess, addresses);
    }
}
