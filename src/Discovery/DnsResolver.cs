...
internal sealed class DnsResolver : PollingResolver
{
    ...
    internal ISystemClock _systemClock = SystemClock.Instance;
    ...
    protected override async Task ResolveAsync(CancellationToken cancellationToken)
    {
        try
        {
            var elapsedTimeSinceLastRefresh = _systemClock.UtcNow - _lastResolveStart;
            ...
            var lastResolveStart = _systemClock.UtcNow;
            ...
            var endpoints = addresses.Select(a =>
            {
                ...
                address.Attributes.Set(ProtocolHelpers.HostOverrideKey, hostOverride);
                return address;
            }).ToArray();
        ...
        catch (Exception ex)
        {
            ...
            Listener(ResolverResult.ForFailure(ProtocolHelpers.CreateStatusFromException(message, ex, StatusCode.Unavailable)));
        }
    }
...
public sealed class DnsResolverFactory : ResolverFactory
{
    ...
    public override Resolver Create(ResolverOptions options)
    {
        ...
        var randomGenerator = GetServiceOrDefault<IRandomGenerator>(serviceProvider, new RandomGenerator());
        var backoffPolicyFactory = GetServiceOrDefault<IBackoffPolicyFactory>(serviceProvider, new ExponentialBackoffPolicyFactory(randomGenerator, options.InitialReconnectBackoff, options.MaxReconnectBackoff));

        return new DnsResolver(options.Address, options.DefaultPort, options.LoggerFactory, _refreshInterval, backoffPolicyFactory);

        static T GetServiceOrDefault<T>(IServiceProvider serviceProvider, T defaultValue)
        {
